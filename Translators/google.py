...
class Google(Translator):
    def __init__(self,from_lan_id,to_lan_id,trans_engine,max_bytes_length):
        ...
        super(Google, self).__init__(from_lan_id, to_lan_id, self.lan_dict, trans_engine, max_bytes_length)
    ...
    def translate(self, from_lan, to_lan, text):
        ...
        result = self.getHtml(self.session, url, self.headers, None, is_post=False)
        if result != None:
            ans = []
            s=''
            for i in result[0]:
...
if __name__ == '__main__':
    ...
    for i in aim_lan:
        if i == 1:
            gg = Google(from_lan_id=i, to_lan_id=2,trans_engine='谷歌',max_bytes_length=2000)
            gg.run()
        else:
            gg = Google(from_lan_id=i,to_lan_id=1,trans_engine='谷歌',max_bytes_length=2000)
            gg.run()
        del gg
